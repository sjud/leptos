...
<br>
If you go to localhost -> you'll get App-1, <br>
and if you go to localhost/app2 -> you'll get app2.
<br>
The two shared servers can be communicated with via actions and local resources, or resources (if using CSR).
<br>
`create_resource` Won't really work with SSR, when trying to communicate to different servers. It will instead try to run the server function (that you've imported) on the server you are serving your server side rendered content from. This will cause errors if your server function relies on state that is not present.
<br>
